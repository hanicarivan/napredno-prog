...
bool ifPasswordExists(UnicodeString Username, UnicodeString Password){
	TLocateOptions Opts;
	Opts.Clear();
	PasswordHashing PasswordExists(Username, Password);
	for(int i = 97; i < 123; i++){
		if(DMUsers->TUsers->Locate("password", PasswordExists.HashLoginPassword(i), Opts)){
			return true;
		}
	}
	return false;
...
void __fastcall TFLogin::OpenMainForm(TObject *Sender)
{
	...
	PasswordHashing LoginInfo(EUsernameLogin->Text, EPasswordLogin->Text);
	if(ifPasswordExists(EUsernameLogin->Text, EPasswordLogin->Text)){
		for(int i = 97; i < 123; i++){
			if(DMUsers->TUsers->Locate("password", LoginInfo.HashLoginPassword(i), Opts)){
				VAR[1] = Variant(LoginInfo.HashLoginPassword(i));
				break;
			}
		}
	}
    Variant LookupResults;
	LookupResults = DMUsers->TUsers->Lookup("username", EUsernameLogin->Text, "admin");
	if(DMUsers->TUsers->Locate("username;password",VarArrayOf(VAR,1),Opts) && (LookupResults == 0)){
		Hide();
		FMain->LActiveUser->Caption =  FLogin->EUsernameLogin->Text;
		FMain->ShowModal();
	}
	else if (DMUsers->TUsers->Locate("username;password",VarArrayOf(VAR,1),Opts) && (LookupResults == 1)){
		Hide();
        FAdmin->ShowModal();
	}
	else {
...
//---------------------------------------------------------------------------
void __fastcall TFLogin::BtnForgotPassClick(TObject *Sender)
{
	Hide();
	FForgot->ShowModal();
}
